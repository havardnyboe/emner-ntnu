...
void RobotGrid::draw_grid_lines() {
    ...
    // NB! FIKK IKKE KOMPILERT PROGRAMMET (SE OPPGAVE G3)!!
    for (int i = 0; i < w_size / cell_width; i++) { // finner antall celler og tegner vertikale linjer
        window.draw_line(Point{x_pos + (i * cell_width), y_pos},
                         Point{x_pos + (i * cell_width), h_size});
    }

    for (int i = 0; i < h_size / cell_height; i++) { // finner antall celler og tegner horisontale linjer
        window.draw_line(Point{x_pos, y_pos + (i * cell_height)},
                         Point{w_size, y_pos + (i * cell_height)});
    }
    // tegner de to siste kantlinjene
    window.draw_line(Point{x_pos, h_size}, Point{w_size, h_size});
    window.draw_line(Point{w_size, h_size}, Point{w_size, y_pos});
...
Point RobotGrid::get_grid_cell_center_coord(int x, int y) const {
    ...
    return {x_pos + (x * cell_width) - (cell_width / 2),
...
void RobotGrid::make_robot(string name, Point pos, Color color) {
    ...
    unique_ptr<Robot> rob{new Robot("rob", {1, 1}, Color::blue)};
    robots.insert({rob->name, rob}); // får feilmeldingen:
    // no instance of overloaded function "std::__1::map<_Key, _Tp, _Compare, _Allocator>::insert [with _Key=std::__1::string, _Tp=std::__1::unique_ptr<Robot, std::__1::default_delete<Robot>>, _Compare=std::__1::less<std::__1::string>, _Allocator=std::__1::allocator<std::__1::pair<const std::__1::string, std::__1::unique_ptr<Robot, std::__1::default_delete<Robot>>>>]" matches the argument list
    // ser ikke feilen med denne impementeringa og får da ikke testa resten av oppgavene siden de er avhengige av mapet og ikke får kompilert.
    // END: G3
}
...
void RobotGrid::draw_robots() {
    ...
    for (pair<string, unique_ptr<Robot>> r : robots) {
        window.draw_circle(get_grid_cell_center_coord(r->pos.x, r->pos.y), cell_width / 2, r->color);
        window.draw_text(get_grid_cell_center_coord(r->pos.x, r->pos.y), r->name, Color::black);
    }
...
void RobotGrid::check_coord_bounds(Point p) const {
    ...
    if (!(p.x >= 0 && p.x < cols && p.y >= 0 && p.y < rows)) {
        throw out_of_range("Koordinatene du skrev inn er utenfor rekkevidden.");
    }
    // END: G10
}
...
void RobotGrid::check_name_available(string name) const {
    ...
    for (pair<string, unique_ptr<Robot>> r : robots) {
        if (name == r.first) {
            throw("Navnet er allerede i bruk.");
        }
    }
    // END: G11
}
...
void RobotGrid::check_name_exists(string name) const {
    ...
    for (pair<string, unique_ptr<Robot>> r : robots) {
        if (!(name == r.first)) {
            throw("Roboten du leter etter eksisterer ikke.");
        }
    }
    // END: G12
}
...
void RobotGrid::check_coord_empty(Point p, string name, bool is_moving) const {
    ...
    // already there and replace it with your own.
    for (pair<string, unique_ptr<Robot>> r : robots) {
        if (p == r.second->pos && !is_moving && name != r.second->name) {
            throw("Det er en annen robot på denne posisjonen.");
        }
    }
    // END: G13
}
...
void RobotGrid::make_grid(int rows, int cols) {
    ...
    this->rows = rows;
    this->cols = cols;
    cell_width = target_w_size / rows;
    cell_height = target_h_size / cols;
    w_size = cell_width * rows;
    h_size = cell_height * cols;
